...
<body>
	...
	<table border="1" width="80%" align="center">
		...
		[<a href="queryStu?pageId=1">首页</a>] | 
		<a href="queryStu?pageId=${ stuManager.currentPage -1 }">上一页</a>
		</c:if>
		<span>
			<c:forEach var="i" begin="1" end="${ stuManager.totalPages }">
				...
				<c:if test="${ i != stuManager.currentPage }">
					<a href="queryStu?pageId=${ i }">  ${ i }  </a>
				</c:if>
			</c:forEach>
		</span>
		<c:if test="${ stuManager.currentPage == stuManager.totalPages}">下一页 | [尾页]</c:if>
		<c:if test="${ stuManager.currentPage != stuManager.totalPages}">
		<a href="queryStu?pageId=${ stuManager.currentPage +1 }">下一页</a> | 
		[<a href="queryStu?pageId=${ stuManager.totalPages }">尾页</a>]
		</c:if>
		</td>
		</tr>
	</table>
